...
from gensim.models import FastText, Word2Vec, LdaMulticore, doc2vec, LsiModel, EnsembleLda, HdpModel
...
class BuildEmbeddings(object):
    def __init__(self,config):
        ...
        outputs_model = config['outputs']['outputs_folder']
        model_folder = config['outputs']['models_folder']
        self.saving_folder = os.path.join(outputs_model,model_folder)
    ...
    def train_model(self,corpus, method = 'fasttext'):
        ...
        self.dictionary.filter_extremes(no_below=self.config['no_below'], no_above= self.config['no_above'])
        ...
        if self.method == 'doc2vec':
            try :
                ...
                tagged_corpus = [doc2vec.TaggedDocument(words= _d, tags=[str(i)]) for i, _d in enumerate(corpus)]
                ...
                self.logger_.warning(f'Fail to train {self.method} model because of {e} ')
            ...
            try :
                ...
                model = LsiModel(self.corpora, id2word=self.dictionary, **self.config['lsi'])
                self.model = model
            ...
            try :
                ...
                model = LdaMulticore(self.corpora, id2word=self.dictionary, **self.config['lda'])
                self.model = model
            ...
            try :
                ...
                model = EnsembleLda(corpus = self.corpora, id2word=self.dictionary, **self.config['elda'])
            ...
            try :
                ...
                model = HdpModel(corpus=self.corpora, id2word=self.dictionary, **self.config['hdp'])
                self.model = model
            ...
            self.logger_.warning(""" The method you specified is not supported yet !
    ...
    def save(self,model_name = None):
        ...
        if self.model is None :
            self.logger_.warning('You are about to save a model that is not instantiated')
            return False
        else: 
            ...
            try :
                shutil.copy(os.path.join(current_folder,'config.yaml'),os.path.join(spe_folder,'config.yaml'))
            except  Exception as e:
            ...
            if self.method in ['fasttext','word2vec','doc2vec'] :
                ...
                self.model.save(os.path.join(spe_folder,model_name+'.model'))
                corpora.MmCorpus.serialize(os.path.join(spe_folder,model_name+'.mm'), self.corpora)
    ...
    def load_model(self,model_name, method):
        model_path = os.path.join(self.saving_folder,method,model_name,model_name+'.model')
        dictionary = None
        corpus = None
